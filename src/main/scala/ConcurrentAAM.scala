...
class ConcurrentAAM[Exp : Expression, Abs : AbstractValue, Addr : Address, Time : Timestamp, TID : ThreadIdentifier](exploration: ExplorationType)
  ...
  class LocalEffectsMap(m: Map[EffectTarget, Set[(Effect[Addr, Abs], TID, State)]]) {
  ...
  object LocalEffectsMap {
    def apply(): LocalEffectsMap = new LocalEffectsMap(Map[EffectTarget, Set[(Effect[Addr, Abs], TID, State)]]())
  }
