...
object Concrete {
  sealed trait L[+X] {
    def foldMap[Y : Lattice](f: X => Y): Y = this match {
      ...
      case Values(content) => content.foldLeft(Lattice[Y].bottom)((acc, v) => Lattice[Y].join(acc, f(v)))
    }
  ...
  abstract class BaseInstance[A : Show](typeName: String) extends Lattice[L[A]] {
    ...
    def eql[B2 : BoolLattice](x: L[A], y: L[A]): B2 = y.guardBot { x.foldMap(a => y.foldMap(b => BoolLattice[B2].inject(a == b))) }
  }
  ...
  object L {
    ...
    implicit val stringConcrete: StringLattice[S] = new BaseInstance[String]("Str") with StringLattice[S] {
      ...
      def lt[B2 : BoolLattice](s1: S, s2: S): B2 = (s1, s2) match {
        ...
        case (Values(content1), Values(content2)) => content1.foldMap(s1 => content2.foldMap(s2 => BoolLattice[B2].inject(s1 < s2)))
      }
    ...
    implicit val intConcrete: IntLattice[I] = new BaseInstance[Int]("Int") with IntLattice[I] {
      ...
      def div[R2 : RealLattice](n1: I, n2: I): R2 = n2.guardBot { n1.foldMap(n1 => n2.foldMap(n2 => RealLattice[R2].inject(n1 / n2.toDouble))) }
      def modulo(n1: I, n2: I): I = n2.guardBot { n1.foldMap(n1 => n2.map(n2 => MathOps.modulo(n1, n2))) }
      def remainder(n1: I, n2: I): I = n2.guardBot { n1.foldMap(n1 => n2.map(n2 => MathOps.remainder(n1, n2))) }
      def lt[B2 : BoolLattice](n1: I, n2: I): B2 = n2.guardBot { n1.foldMap(n1 => n2.foldMap(n2 => BoolLattice[B2].inject(n1 < n2))) }
    ...
    implicit val realConcrete: RealLattice[R] = new BaseInstance[Double]("Real") with RealLattice[R] {
      ...
      def tan(n: R): R = n.map(n => scala.math.sin(n) / scala.math.cos(n)) /* scala.math.tan isn't precise enough */
