...
trait RealLattice[R] extends Lattice[R] { self =>
  ...
  class RealLatticeLaw[I, B, S](implicit val intLat: IntLattice[I],
                                boolLat: BoolLattice[B],
                                strLat: StringLattice[S])
      extends LatticeLaw {
    ...
    def logIsSound(a: Double): Boolean =
      conditional(a > 0, subsumes(log(inject(a)), inject(scala.math.log(a))))
    ...
    def divIsSound(a: Double, b: Double): Boolean =
      conditional(b != 0, subsumes(div(inject(a), inject(b)), inject(a / b)))
