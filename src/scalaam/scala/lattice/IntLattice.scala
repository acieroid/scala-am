...
trait IntLattice[I] extends Lattice[I] { self =>
  ...
  class IntLatticeLaw[R, B, S](implicit val realLat: RealLattice[R],
    ...
    def quotientIsSound(a: Int, b: Int): Boolean =
      conditional(b != 0, subsumes(quotient(inject(a), inject(b)), inject(a / b)))
    def moduloPreservesBottom(a: I): Boolean =
      modulo(a, bottom) == bottom && conditional(!subsumes(a, inject(0)),
