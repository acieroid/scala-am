 ...
 */
  ...
  case class Elements(vs: Set[Value]) extends L {
    ...
    def foldMapL[X](f: Value => X)(implicit monoid: Monoid[X]): X = vs.foldLeft(monoid.zero)((acc, x) => monoid.append(acc, f(x)))
  }
  ...
  implicit val lMonoid = new Monoid[L] {
    def append(x: L, y: => L): L = x match {
      ...
      case Element(a) => y match {
        case Element(Bot) => x
        case Element(b) => Value.join(a, b) match {
  ...
  val schemeLattice: SchemeLattice[L, Exp, A] = new SchemeLattice[L, Exp, A] {
    ...
    def binaryOp(op: BinaryOperator)(x: L, y: L): MayFail[L, Error] = x.foldMapL(x => y.foldMapL(y => Value.binaryOp(op)(x, y).map(x => Element(x))))
    def join(x: L, y: => L): L = Monoid[L].append(x, y)
    def subsumes(x: L, y: => L): Boolean = y.foldMapL(y =>
