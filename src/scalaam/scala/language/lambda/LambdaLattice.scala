...
case class LambdaSetLattice[A <: Address, B : BoolLattice]() {
  ...
  object L {
    ...
    implicit val typeclass = new LambdaLattice[L, A] {
      ...
      def function(e: LambdaExp, env: Environment[A]) = L(Set(Closure(e, env)), BoolLattice[B].bottom)
      ...
      def subsumes(x: L, y: => L) = y.vals.subsetOf(x.vals) && BoolLattice[B].subsumes(x.bools, y.bools)

      /** We can define equality between two lattice elements by using the subsumes relation */
      def eql[B2: BoolLattice](x: L, y: L): B2 = ??? // BoolLattice[B].inject(y.vals.subsetOf(x.vals))
    }
  }
}
