...
case class Table[V](data: Map[(String,String), V], default: Option[V]) {
   ...
   * @param row     The name of the row
   * @param column  The name of the column
   * @return        The value in the table at the given row and column, or the default value if no such element is present
   *                (throws an exception if the element is not found and no default value is set)
   */
   ...
   **/
  def add(row: String, column: String, elem: V): Table[V] = Table(data + ((row,column) -> elem), default)

  /**
   * Add a default value that is used when an element is not present in the table.
   * @param v   The default value to be installed
   * @return    A new table with the given default value
   */
  ...
  def toLatexString(rows: List[String] = allRows.toList.sorted,
    ...
    val content = extract(rows,columns,rowName, v => format(v).replace("_", "\\_")).map(_.mkString(" & "))
                                                                                   .mkString("","\\\\\n","\\\\\n")                                         
  ...
  def toCSVString(rows: List[String] = allRows.toList.sorted,
                  ...
                  rowName: String = "",
                  format: V => String = _.toString()): String =
    extract(rows,columns,rowName,format).map(_.mkString(","))
                                        .mkString("\n")
