...
object Table {
  ...
  */
  def empty[V]: Table[V] = Table(Map[(String,String), V]())
}

case class Table[V](data: Map[(String,String), V], default: Option[V] = None) {
   ...
   **/
  def add(row: String, column: String, elem: V): Table[V] = Table(data + ((row,column) -> elem))

  /**
   * Add a default value that is returned by 'get' when an element is not present in the table.
