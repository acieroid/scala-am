...
trait SchemeBigStepSemantics extends SchemeModXSemantics {
  ...
  trait BigStepIntra extends SchemeModXSemanticsIntra {
    ...
    private def evalNamedLet(id: Identifier, bindings: List[(Identifier, SchemeExp)], body: List[SchemeExp], idn: Identity): Value = {
      ...
      applyFun(call, closure, argsVals, id.idn.pos, component)
    }
    ...
    private def evalCall(exp: SchemeFuncall, fun: SchemeExp, args: List[SchemeExp]): Value = {
      ...
      applyFun(exp, funVal, args.zip(argVals), fun.idn.pos, component)
    }
...
trait BigStepModFSemantics extends SchemeModFSemantics with SchemeBigStepSemantics {
  // defining the intra-analysis
  override def intraAnalysis(cmp: Component) = new BigStepModFIntra(cmp)
  class BigStepModFIntra(cmp: Component) extends IntraAnalysis(cmp) with SchemeModFSemanticsIntra with BigStepIntra {
