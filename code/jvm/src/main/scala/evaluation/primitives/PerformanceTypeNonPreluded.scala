...
object PerformanceType extends Performance {
  ...
  abstract class AnalysisWithManualPrimitives(p: SchemeExp) extends ModAnalysis(p) with SchemeModFSemantics with BigStepModFSemantics with StandardSchemeModFComponents with LIFOWorklistAlgorithm[SchemeExp] {
  ...
  abstract class AnalysisWithPreludedPrimitives(p: SchemeExp) extends ModAnalysis(p) with SchemeModFSemantics with BigStepModFSemantics with StandardSchemeModFComponents with LIFOWorklistAlgorithm[SchemeExp] {
    override lazy val program = {
      val originalProgram = p
      // We exclude all defined primitives from the preludation
      val preludedProgram = SchemePrelude.addPrelude(originalProgram, primitives.allPrimitives.map(_.name).toSet)
