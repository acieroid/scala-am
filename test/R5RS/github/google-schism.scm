...
;; Source: https://github.com/google/schism (01/09/2020)

;; This file has been modified as follows:
;; * The definitions from rnrs.ss are added on top of this file. Some definitions have been omitted and were added in comments.
;; * The file no longer defines a library.
;; * Various other changes, such as the addition of a main body to the program.

;; Some JS definitions with a % can be found in rt.mjs
;; Dummy implementations:
(define log "")
(define (%log-char c) (set! log (string-append log (string c))))
(define (%gensym? s) (and (string? s)(equal? (string-ref s 0) #\S)))
(define %symbol->string symbol->string)
(define %string->list string->list)
(define %char-value (lambda (x) x))
(define (%flush-log)
  (printToLog log)
  (set! log ""))
(define (error . msgs)
  (for-each printToLog msgs)
  (error msgs))
...
(define (printToLog x)
  (cond
    ...
    ((number? x) (%printToLog-number x))
    ((char? x) (%log-char #\#) (%log-char #\\) (%log-char x))
    ((string? x) (%log-char #\") (%printToLog-raw-string x) (%log-char #\"))
    ((procedure? x)
      (%printToLog-raw-string "#<procedure>"))
    (else (%printToLog-raw-string "<!printToLog unknown unimplemented!>"))))

(define (%printToLog-number n)
  (cond ((< n 0) (%log-char #\-) (%printToLog-leading-digits (- 0 n)))
    ((eq? n 0) (%log-char #\0))
    (else (%printToLog-leading-digits n))))
(define (%printToLog-leading-digits n)
  (if (not (zero? n))
    (%printToLog-leading-digits (div0 n 10))
    (%printToLog-least-significant-digit n)))
(define (%printToLog-least-significant-digit n)
  (%log-char (integer->char (+ (char->integer #\0) (mod0 n 10)))))
(define (%printToLog-pair-tail x)
  (cond
    ((null? x) (%log-char #\)))
    ((pair? x)
      (%log-char #\space)
    ...
    (else
      ...
      (printToLog x)
      (%log-char #\)))))
(define (%printToLog-raw-string s)
  (if (not (string? s))
    (error '%printToLog-raw-string "not a string"))
  (%printToLog-chars-as-string (%string->list s)))
(define (%printToLog-chars-as-string chars)
  (if (not (null? chars))
    (begin (if (not (pair? chars))
             (error '%printToLog-chars-as-string "not a list of chars"))
      (%log-char (%char-value (car chars)))
      (%printToLog-chars-as-string (cdr chars)))))
(define (%printToLog-gensym sym)
  (%log-char #\#) (%log-char #\<)
  (%log-char #\g) (%log-char #\e) (%log-char #\n)
  (%log-char #\s) (%log-char #\y) (%log-char #\m)
  (%log-char #\space)
  (%printToLog-raw-string (%symbol->string sym))
  (%log-char #\>))
(define (%printToLog-symbol sym)
  (%printToLog-raw-string (%symbol->string sym)))

(define (write x)
  (if (string? x)
    (%printToLog-raw-string x)
    (printToLog x)))
...
;      (if (null? ls)
;          #f
;          (begin (printToLog x) (newline) (error 'assoc "not a list")))))
;(define (assp p ls)
;  (if (pair? ls)
;      (if (p (caar ls))
;          (car ls)
...
;      (if (null? ls)
;          #f
;          (begin (printToLog ls) (newline) (error 'assp "not a list")))))
;(define (assq x ls)
;  (if (pair? ls)
;      (if (eq? x (caar ls))
;          (car ls)
...
;      (if (null? ls)
;          #f
;          (begin (printToLog x) (newline) (error 'assq "not a list")))))
(define (find pred ls)
  (and (pair? ls)
    (if (pred (car ls))
      (car ls)
...
;  (if (not (and (string? s1) (string? s2))) (error 'string-append "not a string"))
;  (%string-append s1 s2))
...
(define (parse-definitions definitions env)
  (map (lambda (def) (parse-definition def env)) definitions))
...
(define (simplify-expr expr)
  (let ((tag (car expr)))
    (cond
      ((or (eq? tag 'i32) (eq? tag 'const) (eq? tag 'top)) expr)
      ((eq? tag 'var) expr)
      ((eq? tag 'let)
        (let* ((vars (map car (cadr expr)))
...
(define (annotate-free-vars-expr expr bodies)
  (let ((tag (car expr)))
    (cond
      ...
      ((or (eq? tag 'var) (eq? tag 'nop) (eq? tag 'i32) (eq? tag 'const) (eq? tag 'top))
        expr)
      ((eq? tag 'lambda)
...
(define (find-free-vars expr)
  (let ((tag (car expr)))
    (cond
      ((eq? tag 'var) (cdr expr))
      ((or (eq? tag 'nop) (eq? tag 'i32) (eq? tag 'const) (eq? tag 'top)) '())
...
(define (lower-literals-in-definition def env)
  (if (eq? (car def) 'define)
    (let ((name (cadr def))
           (body (caddr def)))
        `(,name ,(intern-constant name env) ,(lower-literals body env)))
    (let ((def (car def))
           (body (cadr def)))
...
;; -> (fns globals start)
(define (lower-literals-in-definitions fns)
  (let* ((cache '())
          (nglobals 0)
          (start #f)
          (env (cons cache (cons nglobals (cons start '()))))
          (fns (map (lambda (fn) (lower-literals-in-definition fn env)) fns))
          (cache (car env))
          (nglobals (cadr env))
...
(define (build-exports exports names top-level-vars)
  (map (lambda (ex)
         (let ((var (assq ex top-level-vars)))
           (if var
               `(gl ,(cdr var) ,(symbol->string (car var)))
...
(define (encode-export export)
  (cond
    ...
    ((eq? (car export) 'gl)
      (cons (encode-string (caddr export)) (cons #x03 (encode-uleb (cadr export)))))
    (else
...
(define (compile-library library)
  (let* ((libraries (read-imports library))
          ...
          (fns (car fns+vars+globals+start))
          (vars (cadr fns+vars+globals+start))
          (globals (caddr fns+vars+globals+start))
          (start (cadddr fns+vars+globals+start))
          ...
          (exports (build-exports exports fn-names vars)))
    (cons
      (wasm-header)
      (cons
...
;; Todo: perhaps use compile-stdin->stdout function.
(compile-library '(library test (export) (import) (define (plus x y)) (+ x y)))