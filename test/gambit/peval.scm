...
(define (get-last-pair l)
  (let ((x (cdr l))) (if (pair? x) (get-last-pair x) l)))
  ...
  (define (simp! where env)
    ...
    (define (s! where)
      (let ((exp (car where)))
        ...
        (cond ((const-expr? exp) #t)  ; leave constants the way they are

              ((symbol? exp) #t)      ; leave variable references the way they are
              ...
              ((eq? (car exp) 'begin)
               (for-each! s! (cdr exp))
               (letrec ((loop (lambda (exps) ; remove all useless expressions
