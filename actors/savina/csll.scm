...
(define SizePercent (int-top))
(define WritePercent (int-top))
...
(define master (a/actor "master" (workers sortedlist terminated)
                        (create-workers ()
                                        (let ((workers (build-vector NumWorkers (lambda (i) (a/create worker self sortedlist i 0)))))
                                          (vector-foreach (lambda (w) (a/send w do-work)) workers)
                                          (a/become master workers sortedlist terminated)))
                        (end-work ()
                                  (if (= (+ terminated 1) NumMsgsPerWorker)
                                      (begin
...
(define worker (a/actor "worker" (master sortedlist id message-count)
                        (do-work () (worker-work self master sortedlist id message-count))
                        (result (value) (if (<= message-count NumMsgsPerWorker)
                                            (worker-work self master sortedlist id message-count)
                                            (begin
...
(define sortedlist (a/actor "sortedlist" (state)
                            ...
                            (contains (sender value)
                                      (a/send sender result (member value state))
