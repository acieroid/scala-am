  ...
  (let* ((s (list-ref data-sizes n))
    ...
    (letrec ((sor-runner-actor
              (a/actor "sor-runner" (g-total returned total-msg-rcv expecting-boot)
                       (boot ()
                             (boot a/self)
                             (a/become sor-runner-actor g-total returned total-msg-rcv #f))
                       (result (mx my mv msg-rcv)
                               (if expecting-boot (error "not booted yet!") #f)
                               (if (= (+ returned 1) (+ (* s part) 1))
                                   (a/terminate)
                                   (a/become sor-runner-actor (+ g-total mv) (+ returned 1) (+ total-msg-rcv msg-rcv) expecting-boot)))
                       (border (mborder)
                               (if expecting-boot (error "not booted yet!") #f)
                               (letrec ((rloop1i (lambda (i)
                                 ...
                                 (rloop1i 0)
                                 (rloop2i 0))
                               (a/become sor-runner-actor g-total returned total-msg-rcv expecting-boot)))))
      (a/create sor-runner-actor 0 0 0 #t))))
          ...
          (if (and (> x 0) (< x (- nx 1)) (> y 0) (< y (- ny 1)))
              ...
              (if (and (or (= x 0) (= x (- nx 1))) (or (= y 0) (= y (- ny 1))))
                  ...
                  (if (or (or (= x 0) (= x (- nx 1))) (or (= y 0) (= y (- ny 1))))
                      ...
                      '())))))
    (letrec ((sor-actor-actor (a/actor "sor-actor" (value iter max-iter msg-rcv sor-actors received-vals sum expecting-start pending-messages)
                             (start (mi mactors)
                                    (if (= color 1)
                                    ...
                                    (a/become sor-actor-actor value (if (= color 1) (+ iter 1) iter) mi (if (= color 1) (+ msg-rcv 1) msg-rcv)
                                              mactors received-vals sum #f '()))
                             (sor-value (v)
                                        (if expecting-start
                                            (a/become sor-actor-actor value iter max-iter msg-rcv sor-actors received-vals sum expecting-start (cons v pending-messages))
                                            (if (< iter max-iter)
                                                    ...
                                                    (begin
                                                      ...
                                                      (if (= (+ iter 1) max-iter)
                                                          (begin
                                                            ...
                                                            (a/terminate))
                                                          (a/become sor-actor-actor (+ (* omega-over-four sum) (* one-minus-omega value))
                                                                    (+ iter 1) max-iter (+ msg-rcv 1) sor-actors (+ received-vals 1) (+ sum v) expecting-start pending-messages)))
                                                    (a/become sor-actor-actor value iter max-iter (+ msg-rcv 1) sor-actors (+ received-vals 1) (+ sum v) expecting-start pending-messages))
                                                (a/terminate)
                                                ))))))
      (a/create sor-actor-actor value 0 0 0 #f 0 0 #t '()))))
    ...
    (define (boot self)
      (let ((my-border (make-vector s #f)))
        (letrec ((ploop1 (lambda (i)
          ...
          (ploop3i 0))))
    (letrec ((sor-peer-actor (a/actor "sor-peer" (g-total returned total-msg-rcv expecting-boot)
                             (boot ()
                                   (boot a/self)
                                   (a/become sor-peer-actor g-total returned total-msg-rcv #f))
                             (result (mx my mv msg-rcv)
                                     (if expecting-boot (error "not booted yet") #f)
                                     (if (= (+ returned 1) (* s (- s part-start)))
                                         (begin (a/send sor-source result -1 -1 g-total total-msg-rcv)
                                                (a/terminate))
                                         (a/become sor-peer-actor (+ g-total mv) (+ returned 1) (+ total-msg-rcv msg-rcv) expecting-boot))))))
      (a/create sor-peer-actor 0 0 0 #t))))
