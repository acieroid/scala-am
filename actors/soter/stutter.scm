...
(letrec ((stutter (a/actor "stutter" (f)
                         (message (x)
                                  (a/become unstutter f))))
         (unstutter (a/actor "unstutter" (f)
                           (message (x)
                                    (f x)
                                    (a/become stutter f))))
         (p (a/create stutter (lambda (msg) (dosmt msg))))
         (dosmt (lambda (x) (if (= x 0) (error "We abhorr '0's") "we love not-0")))
         (sendA (lambda (p) (a/send p message 0) (sendB p)))
         (sendB (lambda (p) (a/send p message 1) (sendA p))))
  (sendA p))
